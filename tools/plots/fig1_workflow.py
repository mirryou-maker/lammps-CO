...
RISK_COLS = ["#43A047", "#66BB6A", "#1E88E5", "#1565C0", "#0D47A1", "#4A148C"]
...
ax.set_title("(a) Six-step optimization pipeline", fontsize=14, pad=10, fontweight="bold")

steps = [
    ("B-1", "Benchmarking\nInfrastructure",  "No code changes\nAutomates measurement",   13.2, "Very Low"),
    ("B-2", "Coverage\nDiagnostics",          "Script analysis\nno source changes",        10.9, "Very Low"),
    ("A-2", "Compiler Flag\nTuning",          "CMake preset\nno source changes",            8.6, "Low"),
    ("A-1", "OMP Variant\nGeneration",        "35 new files\nbit-identical verified",       6.3, "Low"),
    ("A-3", "restrict/fxtmp\nBackport",       "102 files modified\nbit-identical OK",       4.0, "Low–Med"),
    ("A-4", "pow() Deduplication",            "nm/cut · lj/pirani · mie/cut\n−20–41% speed", 1.7, "Low"),
]
...
ax.text(5, 14.6, "Increasing risk & code invasiveness →",
...
levels = [
    (0.5,  3.0, 4.8, "Level 1: Build-time",
     "Compilation succeeds\nno new MSVC warnings", "#66BB6A"),
    (3.7,  3.0, 3.4, "Level 2: Bit-identical",
     "50 NVE steps: temp, epair,\netotal, press match to 15 sf", "#1E88E5"),
    (6.9,  3.0, 2.1, "Level 3: Conservation",
...
for (yb, h, wh, lab, sub, col) in levels:
    ...
    ax.text(5, yb + h * 0.62, lab, ha="center", va="center",
            fontsize=11.5, fontweight="bold", color=col)
    ax.text(5, yb + h * 0.25, sub, ha="center", va="center",
...
ax.text(9.7, 10.8, "Scope", ha="right", fontsize=12, color="#777", fontweight="bold")

ax.text(5, 10.5, "✓ All 35 new OMP variants\npass all 3 levels",
        ha="center", va="center", fontsize=12, color="#0D47A1",
        bbox=dict(fc="#E8EAF6", ec="#3949AB", boxstyle="round,pad=0.6", lw=1.8))

plt.suptitle(
    "Figure 1 · AI-assisted LAMMPS optimization: workflow, interaction pattern, and verification",
    fontsize=16, y=1.02, fontweight="bold")

out = str(pathlib.Path(__file__).parent / "figure1_workflow.pdf")
