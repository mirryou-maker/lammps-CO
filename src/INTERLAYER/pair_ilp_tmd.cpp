  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype] && atom->molecule[i] != atom->molecule[j]) {
        ...
        fxtmp += fkcx - fprod1[0] * Tap;
        fytmp += fkcy - fprod1[1] * Tap;
        fztmp += fkcz - fprod1[2] * Tap;
        f[j].x -= fkcx;
        f[j].y -= fkcy;
        f[j].z -= fkcz;
...
void PairILPTMD::calc_normal()
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    cont = 0;
    const int * const jlist = ILP_firstneigh[i];
    jnum = ILP_numneigh[i];
    for (jj = 0; jj < jnum; jj++) {
      j = jlist[jj];
      j &= NEIGHMASK;
      ...
      if (strcmp(elements[itype], "Hw") == 0) {
        if(cont == 0) {
          ...
          if (strcmp(elements[iH1], "Ow") == 0) {
            vect[0][0] = x[jH1].x - xtp;
            vect[0][1] = x[jH1].y - ytp;
            vect[0][2] = x[jH1].z - ztp;
          } else if (strcmp(elements[iH2], "Ow") == 0) {
            vect[0][0] = x[jH2].x - xtp;
            vect[0][1] = x[jH2].y - ytp;
            vect[0][2] = x[jH2].z - ztp;
          } else {
      ...
      //############################ For Oxygen in the water molecule #######################
        ...
        if(cont == 0) {
          ...
          if (strcmp(elements[iH1], "Hw") == 0 && strcmp(elements[iH2], "Hw") == 0) {
            ...
            vect[1][0] = x[jH2].x - xtp;
            vect[1][1] = x[jH2].y - ytp;
            vect[1][2] = x[jH2].z - ztp;
