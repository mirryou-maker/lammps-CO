...
void PairCoulShield::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const sigmaei  = sigmae[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype] && atom->molecule[i] != atom->molecule[j]) {
        r = sqrt(rsq);
        r3 = rsq * r;
        rarg = 1.0 / sigmaei[jtype];
        ...
        if (tap_flag) {
          Tap = calc_Tap(r, cuti[jtype]);
          dTap = calc_dTap(r, cuti[jtype]);
        } else {
          Tap = 1.0;
          dTap = 0.0;
        }
