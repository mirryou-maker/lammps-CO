...
void PairSPHLJ::compute(int eflag, int vflag)
{
  ...
  double * _noalias const desph = atom->desph;
  const double * _noalias const esph = atom->esph;
  const double * _noalias const cv = atom->cv;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const viscosityi = viscosity[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (domain->dimension == 3) {
          ...
          // (2) using f[i].x += delx * fpair instead of f[i].x += (delx/r) * fpair
        ...
        delVdotDelR = delx * (vxtmp - v[j].x) + dely * (vytmp - v[j].y)
            + delz * (vztmp - v[j].z);

        // artificial viscosity (Monaghan 1992)
        if (delVdotDelR < 0.) {
          mu = h * delVdotDelR / (rsq + 0.01 * h * h);
          fvisc = -viscosityi[jtype] * (ci + cj) * mu / (rho[i] + rho[j]);
        } else {
          fvisc = 0.;
        }
