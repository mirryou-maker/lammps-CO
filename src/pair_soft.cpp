...
void PairSoft::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const prefactori= prefactor[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        r = sqrt(rsq);
        arg = MY_PI*r/cuti[jtype];
        if (r > 0.0) fpair = factor_lj * prefactori[jtype] *
                       sin(arg) * MY_PI/cuti[jtype]/r;
        else fpair = 0.0;
        ...
        if (eflag)
          evdwl = factor_lj * prefactori[jtype] * (1.0+cos(arg));
