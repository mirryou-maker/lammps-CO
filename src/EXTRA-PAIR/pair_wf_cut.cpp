...
void PairWFCut::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const int    * _noalias const mui       = mu[itype];
    const int    * _noalias const nui       = nu[itype];
    const double * _noalias const nmi       = nm[itype];
    const double * _noalias const e0nmi     = e0nm[itype];
    const double * _noalias const rcmui     = rcmu[itype];
    const double * _noalias const sigma_mui = sigma_mu[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        r2inv = 1.0/rsq;
        rminv = powint(r2inv,mui[jtype]);
        rm = sigma_mui[jtype]*rminv - 1.0;
        rn = rcmui[jtype]*rminv - 1.0;

        forcenm = 2.0*mui[jtype] *sigma_mui[jtype]*powint(rn,2*nui[jtype])
                + 4.0*nmi[jtype] *rcmui[jtype]*rm*powint(rn,2*nui[jtype]-1);
        fpair = factor_lj*e0nmi[jtype]*forcenm*powint(r2inv,mui[jtype]+1);
        ...
        if (eflag) {
          evdwl = e0nmi[jtype] * (rm*powint(rn,2*nui[jtype]));
          evdwl *= factor_lj;
        }
