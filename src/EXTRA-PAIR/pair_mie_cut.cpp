...
void PairMIECut::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const gamAi   = gamA[itype];
    const double * _noalias const gamRi   = gamR[itype];
    const double * _noalias const mie1i   = mie1[itype];
    const double * _noalias const mie2i   = mie2[itype];
    const double * _noalias const mie3i   = mie3[itype];
    const double * _noalias const mie4i   = mie4[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        r2inv = 1.0/rsq;
        rgamA = pow(r2inv,(gamAi[jtype]/2.0));
        rgamR = pow(r2inv,(gamRi[jtype]/2.0));
        forcemie =  (mie1i[jtype]*rgamR - mie2i[jtype]*rgamA);
        ...
        if (eflag) {
          evdwl = (mie3i[jtype]*rgamR - mie4i[jtype]*rgamA) -
            offseti[jtype];
          evdwl *= factor_mie;
        }
...
void PairMIECut::compute_inner()
{
  ...
  const double * _noalias const special_mie = force->special_lj;
  ...
  const int * const * const firstneigh = list->firstneigh_inner;
...
void PairMIECut::compute_middle()
{
  ...
  const int * const * const firstneigh = list->firstneigh_middle;
...
void PairMIECut::compute_outer(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        // pow() computed once and reused across force/eflag/vflag blocks
        r2inv = 1.0/rsq;
        rgamA = pow(r2inv,(gamAi[jtype]/2.0));
        rgamR = pow(r2inv,(gamRi[jtype]/2.0));
        forcemie = (mie1i[jtype]*rgamR - mie2i[jtype]*rgamA);
        ...
        if (eflag) {
          evdwl = (mie3i[jtype]*rgamR - mie4i[jtype]*rgamA) - offseti[jtype];
          evdwl *= factor_mie;
        }
