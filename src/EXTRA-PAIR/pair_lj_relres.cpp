...
void PairLJRelRes::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const cutf_inner_sqi = cutf_inner_sq[itype];
    const double * _noalias const ljf1i          = ljf1[itype];
    const double * _noalias const ljf2i          = ljf2[itype];
    const double * _noalias const ljf3i          = ljf3[itype];
    const double * _noalias const ljf4i          = ljf4[itype];
    const double * _noalias const cutfsqi        = cutfsq[itype];
    const double * _noalias const ljswf0i        = ljswf0[itype];
    const double * _noalias const ljswf1i        = ljswf1[itype];
    const double * _noalias const ljswf2i        = ljswf2[itype];
    const double * _noalias const ljswf3i        = ljswf3[itype];
    const double * _noalias const ljswf4i        = ljswf4[itype];
    const double * _noalias const cutf_inneri    = cutf_inner[itype];
    const double * _noalias const offsetsmi      = offsetsm[itype];
    const double * _noalias const offsetspi      = offsetsp[itype];
    const double * _noalias const cut_inner_sqi  = cut_inner_sq[itype];
    const double * _noalias const lj1i           = lj1[itype];
    const double * _noalias const lj2i           = lj2[itype];
    const double * _noalias const lj3i           = lj3[itype];
    const double * _noalias const lj4i           = lj4[itype];
    const double * _noalias const ljsw0i         = ljsw0[itype];
    const double * _noalias const ljsw1i         = ljsw1[itype];
    const double * _noalias const ljsw2i         = ljsw2[itype];
    const double * _noalias const ljsw3i         = ljsw3[itype];
    const double * _noalias const ljsw4i         = ljsw4[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        r2inv = 1.0/rsq;
        if (rsq < cutf_inner_sqi[jtype]) {
          r6inv = r2inv*r2inv*r2inv;
          forcelj = r6inv*(ljf1i[jtype]*r6inv-ljf2i[jtype]);
        } else if (rsq < cutfsqi[jtype]) {
          r = sqrt(rsq);
          t = r - cutf_inneri[jtype];
          tsq = t*t;
          fskin = ljswf1i[jtype]+ljswf2i[jtype]*t+
            ljswf3i[jtype]*tsq+ljswf4i[jtype]*tsq*t;
          forcelj = fskin*r;
        } else if (rsq < cut_inner_sqi[jtype]) {
          r6inv = r2inv*r2inv*r2inv;
          forcelj = r6inv*(lj1i[jtype]*r6inv-lj2i[jtype]);
        } else {
          r = sqrt(rsq);
          t = r-cut_inneri[jtype];
          tsq = t*t;
          fskin = ljsw1i[jtype]+ljsw2i[jtype]*t+
            ljsw3i[jtype]*tsq+ljsw4i[jtype]*tsq*t;
          forcelj = fskin*r;
        }
        ...
        if (eflag) {
          if (rsq < cutf_inner_sqi[jtype]) {
            evdwl = r6inv*(ljf3i[jtype]*r6inv-ljf4i[jtype])-offsetsmi[jtype];
          } else if (rsq < cutfsqi[jtype]) {
            evdwl = ljswf0i[jtype]-ljswf1i[jtype]*t-
              ljswf2i[jtype]*tsq/2.0-ljswf3i[jtype]*tsq*t/3.0-
              ljswf4i[jtype]*tsq*tsq/4.0-offsetspi[jtype];
          } else if (rsq < cut_inner_sqi[jtype]) {
            evdwl = r6inv*(lj3i[jtype]*r6inv-lj4i[jtype])-offseti[jtype];
          } else {
            evdwl = ljsw0i[jtype]-ljsw1i[jtype]*t-
              ljsw2i[jtype]*tsq/2.0-ljsw3i[jtype]*tsq*t/3.0-
              ljsw4i[jtype]*tsq*tsq/4.0-offseti[jtype];
          }
          evdwl *= factor_lj;
        }
