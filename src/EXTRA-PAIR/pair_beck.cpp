...
void PairBeck::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const AAi     = AA[itype];
    const double * _noalias const BBi     = BB[itype];
    const double * _noalias const aai     = aa[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        r = sqrt(rsq);
        r5 = rsq * rsq * r;
        const double aaij    = aai[jtype];
        const double alphaij = alphai[jtype];
        const double betaij  = betai[jtype];
        term1 = aaij * aaij + rsq;
        ...
        rinv = 1.0 / r;
        force_beck = AAi[jtype] * exp(-1.0 * r * term4) * term5;
        force_beck -= BBi[jtype] * r * term2 * term3;
        ...
        if (eflag) {
          ...
          term1inv = 1.0 / term1;
          evdwl = AAi[jtype] * exp(-1.0 * r * term4);
          evdwl -= BBi[jtype] * term6 * (1.0 + (2.709 + 3.0 * aaij * aaij) * term1inv);
          evdwl *= factor_lj;
        }
