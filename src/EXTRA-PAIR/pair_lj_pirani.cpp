  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        rx = r / rmi[jtype];
        n_x = alphai[jtype] * rx * rx + betai[jtype];
        pow_rx_n_x = pow(1.0 / rx, n_x);
        pow_rx_gamma = pow(1.0 / rx, gammai[jtype]);

        filj1 = -2.0 * alphai[jtype] * gammai[jtype] * rx * pow_rx_n_x /
            (square(n_x - gammai[jtype]) * rmi[jtype]);

        filj2 = +2.0 * alphai[jtype] * rx * n_x * pow_rx_gamma /
            (square(n_x - gammai[jtype]) * rmi[jtype]);

        filj3 = -2.0 * alphai[jtype] * rx * pow_rx_gamma /
            (rmi[jtype] * (n_x - gammai[jtype]));

        filj4 = +2.0 * alphai[jtype] * gammai[jtype] * (rx / rmi[jtype]) *
            log(1 / rx) * pow_rx_n_x / (n_x - gammai[jtype]);

        filj5 = -1.0 * gammai[jtype] * n_x * pow_rx_n_x / (r * (n_x - gammai[jtype]));

        filj6 = +1.0 * gammai[jtype] * n_x * pow_rx_gamma / (r * (n_x - gammai[jtype]));
        ...
        if (eflag) {
          ilj1 = epsiloni[jtype] * gammai[jtype] * pow_rx_n_x /
              (n_x - gammai[jtype]);
          ilj2 = -epsiloni[jtype] * n_x * pow_rx_gamma /
              (n_x - gammai[jtype]);
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (eflag) {
          ...
          ilj1 = epsiloni[jtype] * gammai[jtype] * pow(1 / rx, n_x) /
              (n_x - gammai[jtype]);
          ilj2 = -epsiloni[jtype] * n_x * pow(1 / rx, gammai[jtype]) /
              (n_x - gammai[jtype]);
