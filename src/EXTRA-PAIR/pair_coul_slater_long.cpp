  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_coulsq) {
        ...
        slater_term = slater_exp*(1.0 + 2.0*rlamda*(1.0 + rlamda));
        ...
        if (eflag) {
          ecoul = prefactor*(erfc - (1.0 + rlamda)*slater_exp);
          if (factor_coul < 1.0) ecoul -= (1.0-factor_coul)*prefactor*(1.0-(1.0 + rlamda)*slater_exp);
        }
