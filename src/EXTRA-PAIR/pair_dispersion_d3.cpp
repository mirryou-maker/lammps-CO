...
void PairDispersionD3::calc_coordination_number()
{
  ...
  for (int ii = 0; ii < inum; ii++) {
    ...
    int jnum = numneigh[i];
    const double xi_x = x[i].x, xi_y = x[i].y, xi_z = x[i].z;
    const double rcov_i = rcov[itype];
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      double delrj_x = xi_x - x[j].x;
      double delrj_y = xi_y - x[j].y;
      double delrj_z = xi_z - x[j].z;

      double rsq = delrj_x * delrj_x + delrj_y * delrj_y + delrj_z * delrj_z;

      // if the atoms are too far away don't consider the contribution
      if (rsq > cn_thr) continue;

      double rr = sqrt(rsq);
      double rcov_ij = (rcov_i + rcov[jtype]) * autoang;
...
void PairDispersionD3::compute(int eflag, int vflag)
{
  ...
  for (int ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const r0ab_i = r0ab[itype];
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        double *c6_res = get_dC6(itype, jtype, cn[i], cn[j]);
  ...
  for (int ii = 0; ii < inum; ii++) {
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (rsq < cn_thr) {
          double rcovij = (rcov_i + rcov[jtype]) * autoang;
