...
void PairPedone::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const pedone1i = pedone1[itype];
    const double * _noalias const pedone2i = pedone2[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        fpair = pedone1i[jtype] * (dexp * dexp - dexp) / r +
            pedone2i[jtype] * r6inv * r6inv * r2inv;
        fpair *= factor_lj;
        ...
        if (eflag) {
          evdwl = d0i[jtype] * (dexp * dexp - 2.0 * dexp) + c0i[jtype] * r6inv * r6inv -
              offseti[jtype];
          evdwl *= factor_lj;
        }
