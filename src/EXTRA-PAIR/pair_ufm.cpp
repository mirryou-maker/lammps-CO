...
void PairUFM::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const uf1i    = uf1[itype];
    const double * _noalias const uf2i    = uf2[itype];
    const double * _noalias const uf3i    = uf3[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        expuf = exp(- rsq * uf2i[jtype]);
        fpair = factor * scalei[jtype] * uf1i[jtype] * expuf /(1.0 - expuf);
        ...
        if (eflag) {
          evdwl = -uf3i[jtype] * log(1.0 - expuf) - offseti[jtype];
            evdwl *= factor;
        }
