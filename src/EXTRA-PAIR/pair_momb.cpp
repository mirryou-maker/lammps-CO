...
void PairMomb::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const rri      = rr[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        ddexp = exp(-dscale * (r/rri[jtype] - 1.0));
        invexp = 1.0/(1+ddexp);
        fpair = morse1i[jtype] * (dexp*dexp - dexp) / r;
        fpair -= sscale*ci[jtype]
          * (invexp*invexp*ddexp*(-dscale/rri[jtype])*r6inv) / r;
        fpair -= sscale*ci[jtype] * (6.0 * invexp * r6inv * r2inv);
        fpair *= factor_lj;
        ...
        if (eflag) {
          evdwl = d0i[jtype] * (dexp*dexp - 2.0*dexp) -
            sscale*ci[jtype]*r6inv*invexp  - offseti[jtype];
          evdwl *= factor_lj;
        }
