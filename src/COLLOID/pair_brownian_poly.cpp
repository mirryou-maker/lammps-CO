...
void PairBrownianPoly::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (flagfld) {
      fxtmp += prethermostat*sqrt(R0*radi)*(random->uniform()-0.5);
      fytmp += prethermostat*sqrt(R0*radi)*(random->uniform()-0.5);
      fztmp += prethermostat*sqrt(R0*radi)*(random->uniform()-0.5);
      if (flaglog) {
        const double radi3 = radi*radi*radi;
        torxtmp += prethermostat*sqrt(RT0*radi3) *
          (random->uniform()-0.5);
        torytmp += prethermostat*sqrt(RT0*radi3) *
          (random->uniform()-0.5);
        torztmp += prethermostat*sqrt(RT0*radi3) *
          (random->uniform()-0.5);
      }
    }
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (r < cut_inneri[jtype])
          h_sep = cut_inneri[jtype] - radi-radj;
