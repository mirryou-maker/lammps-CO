...
void PairColloid::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const int    * _noalias const formi  = form[itype];
    const double * _noalias const lj1i   = lj1[itype];
    const double * _noalias const lj2i   = lj2[itype];
    const double * _noalias const lj3i   = lj3[itype];
    const double * _noalias const lj4i   = lj4[itype];
    const double * _noalias const a12i   = a12[itype];
    const double * _noalias const a1i    = a1[itype];
    const double * _noalias const a2i    = a2[itype];
    const double * _noalias const offseti   = offset[itype];
    const double * _noalias const sigma3i   = sigma3[itype];
    const double * _noalias const sigma6i   = sigma6[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      case SMALL_SMALL:
        ...
        if (eflag) evdwl = r6inv*(r6inv*lj3i[jtype]-lj4i[jtype]) -
                     offseti[jtype];
        break;
      ...
      case LARGE_LARGE:
        ...
        fR = a12i[jtype]*sigma6i[jtype]/r/37800.0;
        evdwl = fR * (h[0]-h[1]-h[2]+h[3]);
        dUR = evdwl/r + 5.0*fR*(g[0]+g[1]-g[2]-g[3]);
        dUA = -a12i[jtype]/3.0*r*((2.0*K[0]*K[7]+1.0)*K[7] +
                                        (2.0*K[0]*K[8]-1.0)*K[8]);
        fpair = factor_lj * (dUR+dUA)/r;
        if (eflag)
          evdwl += a12i[jtype]/6.0 *
            (2.0*K[0]*(K[7]+K[8])-log(K[8]/K[7])) - offseti[jtype];
        if (r <= K[1])
