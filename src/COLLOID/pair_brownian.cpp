...
void PairBrownian::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (flagfld) {
      fxtmp += prethermostat * sqrt(R0) * (random->uniform() - 0.5);
      fytmp += prethermostat * sqrt(R0) * (random->uniform() - 0.5);
      fztmp += prethermostat * sqrt(R0) * (random->uniform() - 0.5);
      if (flaglog) {
        torxtmp += prethermostat * sqrt(RT0) * (random->uniform() - 0.5);
        torytmp += prethermostat * sqrt(RT0) * (random->uniform() - 0.5);
        torztmp += prethermostat * sqrt(RT0) * (random->uniform() - 0.5);
      }
    }
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (r < cut_inneri[jtype]) h_sep = cut_inneri[jtype] - 2.0 * radi;
