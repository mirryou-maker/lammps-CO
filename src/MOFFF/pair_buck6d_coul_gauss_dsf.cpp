...
void PairBuck6dCoulGaussDSF::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const alpha_iji    = alpha_ij[itype];
    const double * _noalias const f_shift_iji  = f_shift_ij[itype];
    const double * _noalias const e_shift_iji  = e_shift_ij[itype];
    const double * _noalias const buck6d1i     = buck6d1[itype];
    const double * _noalias const buck6d2i     = buck6d2[itype];
    const double * _noalias const buck6d3i     = buck6d3[itype];
    const double * _noalias const buck6d4i     = buck6d4[itype];
    const double * _noalias const c0i          = c0[itype];
    const double * _noalias const c1i          = c1[itype];
    const double * _noalias const c2i          = c2[itype];
    const double * _noalias const c3i          = c3[itype];
    const double * _noalias const c4i          = c4[itype];
    const double * _noalias const c5i          = c5[itype];
    const double * _noalias const rsmooth_sqi  = rsmooth_sq[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (rsq < cut_ljsqi[jtype]) {
          ...
          rexp = exp(-r*buck6d2i[jtype]);
          term1 = buck6d3i[jtype]*r6inv;
          term2 = buck6d4i[jtype]*r14inv;
          term3 = term2*term2;
          term4 = 1.0/(1.0 + term2);
          term5 = 1.0/(1.0 + 2.0*term2 + term3);
          forcebuck6d = buck6d1i[jtype]*buck6d2i[jtype]*r*rexp;
          forcebuck6d -= term1*(6.0*term4 - term5*14.0*term2);
          ebuck6d = buck6d1i[jtype]*rexp - term1*term4;

          // smoothing term
          if (rsq > rsmooth_sqi[jtype]) {
            rcu = r*rsq;
            rqu = rsq*rsq;
            sme = c5i[jtype]*rqu*r + c4i[jtype]*rqu + c3i[jtype]*rcu +
                  c2i[jtype]*rsq + c1i[jtype]*r + c0i[jtype];
            smf = 5.0*c5i[jtype]*rqu + 4.0*c4i[jtype]*rcu +
                  3.0*c3i[jtype]*rsq + 2.0*c2i[jtype]*r + c1i[jtype];
        ...
        //below: parametrization for erfcc = erf(alpha_iji[jtype]*r);
          arg = alpha_iji[jtype]*r;
          erfcd = MathSpecial::expmsq(arg);
          erfcc = 1 - (MathSpecial::my_erfcx(arg) * erfcd);

          forcecoul = prefactor * ((erfcc/r) - (2.0/MY_PIS*alpha_iji[jtype]*erfcd) +
                                                r*f_shift_iji[jtype]) * r;
        ...
        if (eflag) {
          if (rsq < cut_ljsqi[jtype]) {
            evdwl = ebuck6d - offseti[jtype];
            evdwl *= factor_lj;
          } else evdwl = 0.0;

          if (rsq < cut_coulsq) {
            ecoul = prefactor * (erfcc - r*e_shift_iji[jtype] -
                                 rsq*f_shift_iji[jtype]);
