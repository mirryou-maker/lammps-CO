...
void PairRESquared::compute(int eflag, int vflag)
{
  ...
  auto * _noalias const tor = (dbl3_t *) atom->torque[0];
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        switch (form[itype][jtype]) {
          ...
          case ELLIPSE_SPHERE:
            ...
            torxtmp += ttor[0] * factor_lj;
            torytmp += ttor[1] * factor_lj;
            torztmp += ttor[2] * factor_lj;
            break;
                             ...
                             double /*factor_coul*/, double factor_lj, double &fforce)
{
  ...
  r12[0] = x_arr[j].x - x_arr[i].x;
  r12[1] = x_arr[j].y - x_arr[i].y;
  r12[2] = x_arr[j].z - x_arr[i].z;
