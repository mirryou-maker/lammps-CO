...
void PairSDPDTaitwaterIsothermal::compute (int eflag, int vflag) {
  ...
  const auto * _noalias const v = (dbl3_t *) atom->vest[0];
  ...
  double * _noalias const drho = atom->drho;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        fxtmp += delx * fpair + (velx + delx * delVdotDelR / rsq) * fvisc + f_random[0];
        fytmp += dely * fpair + (vely + dely * delVdotDelR / rsq) * fvisc + f_random[1];
        fztmp += delz * fpair + (velz + delz * delVdotDelR / rsq) * fvisc + f_random[2];

        // and change in density
        drho[i] += jmass * delVdotDelR * wfd;

        if (newton_pair || j < nlocal) {
          f[j].x -= delx * fpair + (velx + delx * delVdotDelR / rsq) * fvisc + f_random[0];
          f[j].y -= dely * fpair + (vely + dely * delVdotDelR / rsq) * fvisc + f_random[1];
          f[j].z -= delz * fpair + (velz + delz * delVdotDelR / rsq) * fvisc + f_random[2];
