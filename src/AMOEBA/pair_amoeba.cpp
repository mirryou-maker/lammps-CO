...
void PairAmoeba::compute(int eflag, int vflag)
{
  ...
  if (amoeba) {
    ...
    for (int i = 0; i < nlocal; i++) {
      ...
      rdn = kred[iclass];
      xred[i][0] = rdn*(x[i].x-x[j].x) + x[j].x;
      xred[i][1] = rdn*(x[i].y-x[j].y) + x[j].y;
      xred[i][2] = rdn*(x[i].z-x[j].z) + x[j].z;
    }
  }
