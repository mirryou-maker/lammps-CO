...
void PairBPMSpring::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const ki      = k[itype];
    const double * _noalias const kai     = ka[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (anharmonic_flag) {
          fa = -kai[jtype] * dr * dr * dr;
          fpair += fa;
          if (eflag) evdwl += -0.25 * fa * dr;
        }

        smooth = rsq / cutsqi[jtype];
        smooth *= smooth;
        smooth *= smooth;
        smooth = 1.0 - smooth;
        delvx = vxtmp - vel[j].x;
        ...
        fpair -= gammai[jtype] * dot * smooth * rinv;
