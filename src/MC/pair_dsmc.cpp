...
void PairDSMC::compute(int /*eflag*/, int /*vflag*/)
{
  ...
  for (int i = 0; i < nlocal; ++i) {
    int xcell = static_cast<int>((x[i].x - domain->boxlo[0])/cellx);
    int ycell = static_cast<int>((x[i].y - domain->boxlo[1])/celly);
    int zcell = static_cast<int>((x[i].z - domain->boxlo[2])/cellz);
