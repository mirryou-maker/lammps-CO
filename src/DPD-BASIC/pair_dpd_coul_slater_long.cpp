...
void PairDPDCoulSlaterLong::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const cut_dpdsqi  = cut_dpdsq[itype];
    const double * _noalias const cut_dpdi    = cut_dpd[itype];
    const double * _noalias const cut_slatersqi = cut_slatersq[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (rsq < cut_dpdsqi[jtype] && r > EPSILON) {
          rinv = 1.0/r;
          delvx = vxtmp - v[j].x;
          delvy = vytmp - v[j].y;
          delvz = vztmp - v[j].z;
          dot = delx*delvx + dely*delvy + delz*delvz;
          wd = 1.0 - r/cut_dpdi[jtype];
          ...
          forcedpd = a0i[jtype]*wd;
          forcedpd -= gammai[jtype]*wd*wd*dot*rinv;
          forcedpd *= factor_dpd;
          forcedpd += factor_sqrt*sigmai[jtype]*wd*randnum*dtinvsqrt;
          forcedpd *= rinv;

          if (eflag) {
            // eng shifted to 0.0 at cutoff
            evdwl = 0.5*a0i[jtype]*cut_dpdi[jtype] * wd*wd;
            evdwl *= factor_dpd;
          }
