...
void PairLJLongCoulLong::compute(int eflag, int vflag)
{
  ...
  double **x = atom->x; double * _noalias const x0 = x[0];
  double **f = atom->f; double * _noalias const f0 = f[0]; double *fi;
  double *q = atom->q;
  int *type = atom->type;
  ...
  for (; ineigh<ineighn; ++ineigh) {                        // loop over my atoms
    ...
    for (; jneigh<jneighn; ++jneigh) {                       // loop over neighbors
      ...
      {
        double fx = d[0]*fpair, fy = d[1]*fpair, fz = d[2]*fpair;
        fxtmp += fx; fytmp += fy; fztmp += fz;
    ...
    fi[0] += fxtmp; fi[1] += fytmp; fi[2] += fztmp;
  }
