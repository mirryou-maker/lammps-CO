...
void PairOxdna2Coaxstk::compute(int eflag, int vflag)
{
  ...
  for (ia = 0; ia < anum; ia++) {
    ...
    for (ib = 0; ib < bnum; ib++) {
      ...
      delr_stkstk[0] = x[a].x + ra_cstk[0] - x[b].x - rb_cstk[0];
      delr_stkstk[1] = x[a].y + ra_cstk[1] - x[b].y - rb_cstk[1];
      delr_stkstk[2] = x[a].z + ra_cstk[2] - x[b].z - rb_cstk[2];
      ...
      delr_bkbk[0] = (x[a].x + ra_cbk[0] - x[b].x - rb_cbk[0]);
      delr_bkbk[1] = (x[a].y + ra_cbk[1] - x[b].y - rb_cbk[1]);
      delr_bkbk[2] = (x[a].z + ra_cbk[2] - x[b].z - rb_cbk[2]);
