...
void PairSNAP::compute(int eflag, int vflag)
{
  ...
  for (int ii = 0; ii < list->inum; ii++) {
    ...
    for (int jj = 0; jj < ninside; jj++) {
      ...
      fxtmp += fij[0]*scale[itype][itype];
      fytmp += fij[1]*scale[itype][itype];
      fztmp += fij[2]*scale[itype][itype];
      f[j].x -= fij[0]*scale[itype][itype];
      f[j].y -= fij[1]*scale[itype][itype];
      f[j].z -= fij[2]*scale[itype][itype];
...
void PairSNAP::compute_bispectrum()
{
  ...
  for (int ii = 0; ii < list->inum; ii++) {
    ...
    const int * _noalias const jlist = list->firstneigh[i];
