...
void PairPeriVES::compute(int eflag, int vflag)
{
  ...
  const auto * _noalias const x0 = (dbl3_t *) atom->x0[0];
  const int * _noalias const type = atom->type;
  int nlocal = atom->nlocal;

  double timestepsize = update->dt;
  const double * _noalias const vfrac = atom->vfrac;
  double * _noalias const s0 = atom->s0;
  ...
  const int * _noalias const npartner = fix_peri_neigh->npartner;
  const double * _noalias const wvolume = fix_peri_neigh->wvolume;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (r < d_ij) {
        ...
        double kshort = (15.0 * 18.0 * bulkmodulus[itype][itype]) /
          (3.141592653589793 * cutsqi[jtype] * cutsqi[jtype]);
        rk = (kshort * vfrac[j]) * (dr / cuti[jtype]);
