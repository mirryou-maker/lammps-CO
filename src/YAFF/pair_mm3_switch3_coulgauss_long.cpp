...
void PairMM3Switch3CoulGaussLong::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const cut_lji   = cut_lj[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (rsq < cut_ljsqi[jtype]) {
          ...
          r = sqrt(rsq);
          expb = lj3i[jtype]*exp(-lj1i[jtype]*r);
          forcelj = expb*lj1i[jtype]*r;
          // Attractive r^-6 part
          r6inv = r2inv*r2inv*r2inv;
          forcelj -= 6.0*lj4i[jtype]*r6inv;
        ...
        if (rsq < cut_ljsqi[jtype]) {
          ...
          evdwl = expb-lj4i[jtype]*r6inv-offseti[jtype];
        } else evdwl = 0.0;

        // Truncation, see Yaff Switch3
        if (truncw>0) {
          if (rsq < cut_ljsqi[jtype]) {
            if (r>cut_lji[jtype]-truncw) {
              trx = (cut_lji[jtype]-r)*truncwi;
