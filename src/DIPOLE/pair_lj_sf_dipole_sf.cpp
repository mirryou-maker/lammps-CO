    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (rsq < cut_coulsqi[jtype]) {
          ...
          rcutcoul2inv=1.0/cut_coulsqi[jtype];
          if (qtmp != 0.0 && q[j] != 0.0) {
        ...
        if (rsq < cut_ljsqi[jtype]) {
          ...
          forceljcut = r6inv*(lj1i[jtype]*r6inv-lj2i[jtype])*r2inv;

          rcutlj2inv = 1.0 / cut_ljsqi[jtype];
          rcutlj6inv = rcutlj2inv * rcutlj2inv * rcutlj2inv;
          forceljsf = (lj1i[jtype]*rcutlj6inv - lj2i[jtype]) *
        ...
        fq = factor_coul*qqrd2e*scalei[jtype];
        ...
        if (eflag) {
          if (rsq < cut_coulsqi[jtype]) {
            ecoul = (1.0-sqrt(rsq/cut_coulsqi[jtype]));
            ecoul *= ecoul;
            ecoul *= qtmp * q[j] * rinv;
            if (mu[i][3] > 0.0 && mu[j][3] > 0.0)
            ...
            ecoul *= factor_coul*qqrd2e*scalei[jtype];
          } else ecoul = 0.0;

          if (rsq < cut_ljsqi[jtype]) {
            evdwl = r6inv*(lj3i[jtype]*r6inv-lj4i[jtype])+
              rcutlj6inv*(6*lj3i[jtype]*rcutlj6inv-3*lj4i[jtype])*
              rsq*rcutlj2inv+
              rcutlj6inv*(-7*lj3i[jtype]*rcutlj6inv+4*lj4i[jtype]);
            evdwl *= factor_lj;
          } else evdwl = 0.0;
        }
