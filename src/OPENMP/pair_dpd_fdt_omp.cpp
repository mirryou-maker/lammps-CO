...
PairDPDfdtOMP::PairDPDfdtOMP(LAMMPS *lmp) :
  PairDPDfdt(lmp), ThrOMP(lmp, THR_PAIR)
{
...
void PairDPDfdtOMP::compute(int eflag, int vflag)
{
  ev_init(eflag,vflag);
  ...
  if (nthreads != comm->nthreads) {
    ...
    // to ensure full compatibility with the serial DPDfdt style
...
void PairDPDfdtOMP::eval(int iifrom, int iito, ThrData * const thr)
{
  int i,j,ii,jj,jnum,itype,jtype;
  double xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  double vxtmp,vytmp,vztmp,delvx,delvy,delvz;
  double rsq,r,rinv,dot,wd,wr,randnum,factor_dpd,gamma_ij;
  ...
  if (splitFDT_flag) {
    ...
    // when a0 == 0 for all type pairs, this branch contributes nothing
    if (a0_is_zero) return;
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (rsq < cutsq[itype][jtype]) {
          ...
          gamma_ij = sigma[itype][jtype]*sigma[itype][jtype]
                     / (2.0*boltz*temperature);

          // conservative force = a0 * wr
          // drag force = -gamma_ij * wd * (delx dot delv) / r
...
double PairDPDfdtOMP::memory_usage()
{
  double bytes = memory_usage_thr();
  bytes += PairDPDfdt::memory_usage();
