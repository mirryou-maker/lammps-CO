    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        if (tabstyle == LOOKUP) {
          ...
          A_i = tb->f[itable];
          A_j = tb->f[jtable];
          fpair = 0.5*(A_i + A_j)*(1.0+3.0*sqrt(rsq)/sqrt(cutsqi[jtype]))*(1.0 - sqrt(rsq)/sqrt(cutsqi[jtype]))*(1.0 - sqrt(rsq)/sqrt(cutsqi[jtype]))*(1.0 - sqrt(rsq)/sqrt(cutsqi[jtype]));
