  ...
  if (loop == 0) {
    ...
    for (ii = 0; ii < inum; ii ++) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        const double delx = x[j].x - xtmp;
        const double dely = x[j].y - ytmp;
        const double delz = x[j].z - ztmp;
        rsq = delx*delx + dely*dely + delz*delz;
...
void PairSMTBQ::Charge()
{
  ...
  if (strcmp(writepot,"true") == 0 && fmod(static_cast<double>(step), Neverypot) == 0.0) {
    ...
    for (ii = 0; ii < inum; ii++) {
      ...
      if (fichierpot) fichierpot<< setprecision(9) <<i <<" "<<itype<<" "<<x[i].x<<" "<<x[i].y
                                <<" "<<x[i].z<<" "<<q[i]<<" "<<potself[i] + potmad[i]<<" "<<potcov[i]
