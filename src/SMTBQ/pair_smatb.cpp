...
void PairSMATB::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ++ii) {
    ...
    ztmp = x[i].z;
    itype = type[i];
    const double * _noalias const cutOffEnd2i = cutOffEnd2[itype];
    const double * _noalias const cutOffStarti = cutOffStart[itype];
    const double * _noalias const cutOffEndi = cutOffEnd[itype];
    const double * _noalias const QSIi = QSI[itype];
    const double * _noalias const qi = q[itype];
    const double * _noalias const r0i = r0[itype];
    const double * _noalias const x5i = x5[itype];
    const double * _noalias const x4i = x4[itype];
    const double * _noalias const x3i = x3[itype];
    ...
    for (jj = 0; jj < jnum; ++jj) {
      ...
      if (dijsq < cutOffEnd2i[jtype]) {
        dij = sqrt(dijsq);
        if (dij < cutOffStarti[jtype]) {
          qsiexpq = (QSIi[jtype] * QSIi[jtype]) *
              exp(2.0 * qi[jtype] * (1.0 - dij / r0i[jtype]));
        } else {
          polyval = dij - cutOffEndi[jtype];
          polyval3 = polyval * polyval * polyval;
          polyval4 = polyval3 * polyval;
          polyval5 = polyval4 * polyval;
          qsiexpq = x5i[jtype] * polyval5 + x4i[jtype] * polyval4 +
              x3i[jtype] * polyval3;
  ...
  for (ii = 0; ii < inum; ++ii) {
    ...
    const double * _noalias const cutOffEndi = cutOffEnd[itype];
    const double * _noalias const r0i = r0[itype];
    const double * _noalias const pi = p[itype];
    const double * _noalias const Ai = A[itype];
    const double * _noalias const qi = q[itype];
    const double * _noalias const QSIi = QSI[itype];
    const double * _noalias const a5i = a5[itype];
    const double * _noalias const a4i = a4[itype];
    const double * _noalias const a3i = a3[itype];
    const double * _noalias const x5i = x5[itype];
    const double * _noalias const x4i = x4[itype];
    const double * _noalias const x3i = x3[itype];
    const int * _noalias const jlist = firstneigh[i];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (dijsq < cutOffEnd2i[jtype]) {
        dij = sqrt(dijsq);
        if (dij < cutOffStarti[jtype]) {
          espo = 1.0 - dij / r0i[jtype];
          aexpp = exp(pi[jtype] * espo) * Ai[jtype];
          Fr = (2.0 * aexpp) * (pi[jtype] / r0i[jtype]);
          qsiexpq = (QSIi[jtype] * QSIi[jtype]) * exp(2.0 * qi[jtype] * espo);
          Fb = -qsiexpq * qi[jtype] / r0i[jtype];
        } else {
          polyval = dij - cutOffEndi[jtype];
          ...
          polyval4 = polyval3 * polyval;
          polyval5 = polyval4 * polyval;
          aexpp = a5i[jtype] * polyval5 + a4i[jtype] * polyval4 +
              a3i[jtype] * polyval3;
          Fr = -2.0 *
              (5.0 * a5i[jtype] * polyval4 + 4.0 * a4i[jtype] * polyval3 +
               3.0 * a3i[jtype] * polyval2);
          qsiexpq = x5i[jtype] * polyval5 + x4i[jtype] * polyval4 +
              x3i[jtype] * polyval3;
          Fb = (5.0 * x5i[jtype] * polyval4 + 4.0 * x4i[jtype] * polyval3 +
                 3.0 * x3i[jtype] * polyval2) * qsiexpq;
        }
