...
void PairCoulCutSoft::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const lam1i = lam1[itype];
    const double * _noalias const lam2i = lam2[itype];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        denc = sqrt(lam2i[jtype] + rsq);
        forcecoul = qqrd2e * lam1i[jtype] * qtmp*q[j] / (denc*denc*denc);
        ...
        if (eflag)
          ecoul = factor_coul * qqrd2e * lam1i[jtype] * qtmp*q[j] / denc;
