    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_bothsq) {
        ...
        if (rsq < cut_coulsq) {
          ...
          denc = sqrt(lj4i[jtype] + rsq);
          prefactor = qqrd2e * lj1i[jtype] * qtmp*q[j] / (denc*denc*denc);
        ...
        if (rsq < cut_ljsq) {
          ...
          forcelj = lj1i[jtype] * epsiloni[jtype] *
          ...
          if (rsq > cut_lj_innersq) {
            ...
            philj = lj1i[jtype] * 4.0 * epsiloni[jtype] *
        ...
        if (eflag) {
          if (rsq < cut_coulsq) {
            prefactor = qqrd2e * lj1i[jtype] * qtmp*q[j] / denc;
            ecoul = prefactor*erfc;
            if (factor_coul < 1.0) ecoul -= (1.0-factor_coul)*prefactor;
          } else ecoul = 0.0;

          if (rsq < cut_ljsq) {
            evdwl = lj1i[jtype] * 4.0 * epsiloni[jtype] *
              (1.0/(denlj*denlj) - 1.0/denlj);
            if (rsq > cut_lj_innersq) {
              switch1 = (cut_ljsq-rsq) * (cut_ljsq-rsq) *
                (cut_ljsq + 2.0*rsq - 3.0*cut_lj_innersq) / denom_lj;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_out_off_sq) {
        ...
        denc = sqrt(lj4i[jtype] + rsq);
        forcecoul = qqrd2e * lj1i[jtype] * qtmp*q[j] / (denc*denc*denc);
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_bothsq) {
        ...
        if (rsq < cut_coulsq) {
          ...
          denc = sqrt(lj4i[jtype] + rsq);
          fprefactor = qqrd2e * lj1i[jtype] * qtmp*q[j] /
            (denc*denc*denc);
        ...
        if (eflag) {
          if (rsq < cut_coulsq) {
            eprefactor = qqrd2e * lj1i[jtype] * qtmp*q[j] / denc;
