...
void PairSpinExchangeBiquadratic::compute(int eflag, int vflag)
{
  ...
  auto * _noalias const fm = (dbl3_t *) atom->fm[0];
  double **sp = atom->sp;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    spi[2] = sp[i][2];
    emag[i] = 0.0;
    const double * _noalias const cut_spin_exchangei = cut_spin_exchange[itype];

    double fxtmp = 0.0, fytmp = 0.0, fztmp = 0.0;
    double fmxtmp = 0.0, fmytmp = 0.0, fmztmp = 0.0;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      local_cut2 = cut_spin_exchangei[jtype]*cut_spin_exchangei[jtype];
...
void PairSpinExchangeBiquadratic::compute_single_pair(int ii, double fmi[3])
{
  ...
  if (locflag == 1) {
    ...
    const int * _noalias const jlist = firstneigh[ii];
    jnum = numneigh[ii];
    const double * _noalias const cut_spin_exchangei = cut_spin_exchange[itype];
