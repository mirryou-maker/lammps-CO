...
void PairTDPD::compute(int eflag, int vflag)
{
  ...
  for (int ii = 0; ii < inum; ii++) {
    ...
    const double * _noalias const cutcci  = cutcc[itype];
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        wc = MAX(0,MIN(1.0,wc));
        double wr = pow(wc, 0.5*poweri[jtype]);
        ...
        if (eflag) {
          evdwl = 0.5*a0i[jtype]*cuti[jtype] * wc*wc;
          evdwl *= factor_dpd;
        }
