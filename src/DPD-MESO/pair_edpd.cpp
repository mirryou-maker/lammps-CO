...
void PairEDPD::compute(int eflag, int vflag)
{
  ...
  for (int ii = 0; ii < inum; ii++) {
    ...
    double ytmp = x[i].y;
    double ztmp = x[i].z;
    double vxtmp = vel[i].x;
    double vytmp = vel[i].y;
    double vztmp = vel[i].z;
    int itype = type[i];
    const int * _noalias const jlist = firstneigh[i];
    int jnum = numneigh[i];
    const double * _noalias const cutsqi   = cutsq[itype];
    const double * _noalias const cuti     = cut[itype];
    const double * _noalias const cutTi    = cutT[itype];
    const double * _noalias const a0i      = a0[itype];
    const double * _noalias const gammai   = gamma[itype];
    const double * _noalias const poweri   = power[itype];
    const double * _noalias const kappai   = kappa[itype];
    const double * _noalias const powerTi  = powerT[itype];
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsqi[jtype]) {
        ...
        double rinv = 1.0/r;
        double delvx = vxtmp - vel[j].x;
        double delvy = vytmp - vel[j].y;
        double delvz = vztmp - vel[j].z;
        ...
        if (eflag) {
          evdwl = 0.5*a0i[jtype]*T_ij*cuti[jtype] * wc*wc;
          evdwl *= factor_dpd;
        }
