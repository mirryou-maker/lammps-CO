      ...
      if (rsq < rcutSq) {
        ...
        if (fabs(dx.dot(normal)) < radius[particle]) {
          ...
          if (r < 1.0 * radius[particle]) {
            ...
            if (particle < nlocal) {
              f[particle].x += x4cp(0) * fpair;
              f[particle].y += x4cp(1) * fpair;
              f[particle].z += x4cp(2) * fpair;
            }

            if (tri < nlocal) {
              f[tri].x -= x4cp(0) * fpair;
              f[tri].y -= x4cp(1) * fpair;
              f[tri].z -= x4cp(2) * fpair;
            }
          ...
          if (r < touch_distance) {
            ...
            x[particle].x = cp(0) + touch_distance * normal(0);
            x[particle].y = cp(1) + touch_distance * normal(1);
            x[particle].z = cp(2) + touch_distance * normal(2);
          }
        }
      }
    }
