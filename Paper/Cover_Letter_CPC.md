...
The exercise also surfaced a defect in unmodified LAMMPS: `pair_style hybrid/overlay kolmogorov/crespi/z` aborts under `-sf omp`, because `hybrid/overlay` is registered under two names and the sub-style tests `force->pair_style` for exact equality. The upstream test suite cannot see this, as the OpenMP branch is skipped when no OMP variant of the sub-style exists. We offer the one-line fix upstream separately.
