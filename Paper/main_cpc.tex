...
upstream separately; the patch, its reproducer, and the regression run
that accompanies it are distributed with the software in
\texttt{upstream-pr/}. 3. \textbf{Bit-identical thermo comparison}: 50
NVE steps with \texttt{thermo\_modify\ format\ float\ \%20.15g}; all
...
carbonate) solvent beads and one Li⁺ ion in a 47.5 × 47.5 × 47.5 Å³
...
The three contributions differ in how readily they can go upstream, and
it is worth separating them. The \texttt{hybrid/overlay} suffix fix is a
defect in current LAMMPS with a one-line correction that brings three
call sites in line with the idiom \texttt{Force::pair\_match()} already
uses; it is offered upstream on its own. The 36 new OpenMP variants
belong in the OPENMP package by construction --- extending accelerator
coverage is what that package is for --- and are prepared for submission
with the accompanying documentation entries. The kernel rewrite is a
different proposition. LAMMPS deliberately keeps its baseline pair
styles plain and confines aliasing-qualified, accumulator-based code to
the accelerator packages: of the files in current LAMMPS that use
\texttt{\_noalias}, 203 are in OPENMP, 36 in INTEL, and five in OPT,
while none are baseline \texttt{src/} styles. Applying that pattern to
225 baseline files is therefore not a patch but a proposal about where
optimized code should live, and it trades some of the readability those
files are maintained for. Whether that trade is worth making across the
whole set is a question for the maintainers rather than for us; we
intend to open it with a small pilot rather than a sweep, and report the
outcome.

Beyond that, future work will target INTEL/OPT package extensions,
KOKKOS ports for the 36 new styles, and MPI communication buffer
optimization. The computational efficiency gains demonstrated here will
also enable prospective high-throughput screening of novel electrolyte
solvent combinations for next-generation Na-ion and Mg-ion batteries,
where desolvation energetics are even more critical than in Li-ion
systems {[}35,36{]}.
